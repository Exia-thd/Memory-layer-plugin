...
| `artifact` | Ingested content; written by `memory ingest`, rarely by hand |
...
```
memory_write  layer=semantic  title="Cap retries at two"  ...
memory_link   from=<decision> to=<incident> type=RESOLVES
```
...
Check `memory_conflicts` first. Recording a decision that contradicts a
